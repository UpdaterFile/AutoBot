...
def take_input(inp_txt = ""):
    sentance = input(inp_txt)
    
    words = sentance.split(' ')
    auto_correct = [correction(word) for word in words]
    corrected_sentance = auto_correct[0]
    
    for word in auto_correct[1:]:
        corrected_sentance += " "+word 

    if corrected_sentance.lower() == sentance.lower():
        return sentance
    else:
        print(("Auto Correcting {} to {} ...").format(sentance, corrected_sentance) )
        return corrected_sentance
...
time.sleep(1)
check_name(username)
sentance = take_input("How is your day going?\n")
time.sleep(1)
print(" . ")
time.sleep(1)
print(" . ")
...
check_good(sentance)
'''
sent_respond = {"Positive": "I am glad to hear that, let me help you find a vehicle.",
                "Negative": " That's not good, hopefully I can make it better."}

nlp = StanfordCoreNLP('http://localhost:9000')
sentiment = get_sentiment(nlp, sentance)
...
while(endconditionmain==False):
    ...
    while (endcondition == False):
        ...
        sentance = take_input("Are you happy with this vehicle?" )
        time.sleep(1)
        print(" . ")
        time.sleep(1)
        print(" . ")
